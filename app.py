...
def load_preprocessing():
    try:
        ...
        scaler = joblib.load('scalerori.pkl')
...
def predict_new_country(model, scaler, pca, new_data, feature_columns):
    """Predict cluster for new country data - FIXED VERSION"""
    
    # Ensure new_data has the same 13 features
    if isinstance(new_data, dict):
        new_df = pd.DataFrame([new_data])
        # Ensure all 13 feature columns are present
        for col in feature_columns:
            if col not in new_df.columns:
                new_df[col] = 0  # Default value for missing features
        
        new_data_array = new_df[feature_columns].values
        # 1. Scale the 13 original features FIRST
        new_data_scaled = scaler.transform(new_data_array)
        
        # 2. Then reduce to 6 PCA components
        new_data_pca = pca.transform(new_data_scaled)
    else:
        new_data_scaled = scaler.transform(new_data.reshape(1, -1))
        new_data_pca = pca.transform(new_data_scaled)
    
    # 3. Predict using the 6 PCA components
    prediction_proba = model.predict(new_data_pca, verbose=0)
    predicted_cluster = np.argmax(prediction_proba, axis=1)[0]
    confidence = np.max(prediction_proba)
...
def predict_new_country_debug(model, scaler, pca, new_data, feature_columns):
    """Debug version to see the transformation steps"""
    ...
    if isinstance(new_data, dict):
        ...
        print(f"1. Input DataFrame shape: {new_df.shape}")
        print(f"2. Feature columns: {new_df.columns.tolist()}")
        
        # Convert to values
        new_data_values = new_df[feature_columns].values
        print(f"3. As NumPy array shape: {new_data_values.shape}")
        
        new_data_scaled = scaler.transform(new_data_values)
        print(f"4. After scaling shape: {new_data_scaled.shape}")
        
        new_data_pca = pca.transform(new_data_scaled)
        print(f"5. After PCA shape: {new_data_pca.shape}")
    else:
        new_data_scaled = scaler.transform(new_data.reshape(1, -1))
        new_data_pca = pca.transform(new_data_scaled)
    ...
    print(f"6. Prediction: cluster={predicted_cluster}, confidence={confidence:.3f}")
